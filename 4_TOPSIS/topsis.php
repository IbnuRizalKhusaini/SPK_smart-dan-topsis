...
class topsis
{
  ...
  public function __construct()
  {
    ...
    // data kriteria
    $kriteria_awal = $db->query('select * from tb_kriteria_awal');
    while ($row = $kriteria_awal->fetchArray()) {
      array_push($this->kriteria_awal, array($row['kriteria'], $row['bobot'], $row['atribut']));
    }

    // data alternatif
    $alternatif = $db->query('select * from tb_alternatif');
    while ($row = $alternatif->fetchArray()) {
      array_push($this->alternatif, array($row['alternatif'], $row['keramahan_dari_pelayan'], $row['kelengkapan_menu'], $row['tempat_parkir_kendaraan'], $row['ketersedian_wifi'], $row['harga']));
    }
